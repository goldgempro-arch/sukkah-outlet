...
/**
 * Generate MULTIPLE reuse options sorted by cost (cheapest first).
 * Returns up to 10 distinct valid reuse combinations.
 */
  ...
  for (const subset of powerset(existingTuples)) {
    ...
    for (const fillRoll of fillRolls) {
      ...
      if (remainingW > 0) {
        ...
        const combo = _solve_width_cheapest(matType, fillRoll, remainingW, [], fedex);
        if (!combo) continue;
        fillCost = combo.reduce((a, c) => a + c.price * c.qty, 0) * fillRows;
        fillMats = combo.map((c) => [c.code, c.width, c.roll, c.price, c.qty, fillRoll, fillRows]);
      }

      // Gap fill for short existing mats
      const gapMatsAll: [string, number, number, number, number, number, number][] = [];
      let gapCostTotal = 0;
      let valid = true;
      ...
      const totalCost = fillCost + gapCostTotal;
      const result = _build_general_reuse_result(matType, subset, fillMats, gapMatsAll, poleFt, totalWidth, existing, fedex, totalCost);

      // Dedup by mat combo signature
